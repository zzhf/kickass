...
(function(window) {
    ...
    var Menu = new Class({
        ...
        generate: function(parent) {
            ...
            // this.url = GameGlobals.path('intermediate.html?url=' + encodeURIComponent(getGlobalNamespace().KICKASSURL || document.location.href) + "&preship=" + (shipId) + "&is_campaign=" + (this.game.isCampaign() ? "true" : "") + "&is_mysite=" + (this.game.isMySite() ? "true" : ""));
            this.url = "";
        ...
        onGameReady: function() {
            ...
            this.socket.postMessage("url:!" + (getGlobalNamespace().KICKASSURL || document.location.href));
    ...
    var MenuManager = new Class({
        ...
        create: function() {
            ...
            // var adHTML = this.game.shouldShowAd() ? '<iframe src="' + GameGlobals.path('hello.html') + '" class="KICKASSELEMENT" id="kickass-hello-sunshine"></iframe>' : "";
            var adHTML = false;
            this.container.innerHTML = '<div id="kickass-howto-image" class="KICKASSELEMENT kickass-howto-invisible"></div>' + '<div id="kickass-pointstab" class="KICKASSELEMENT">' + adHTML + '<div id="kickass-bomb-menu" class="KICKASSELEMENT KICKASShidden">' + '<ul id="kickass-bomb-list" class="KICKASSELEMENT">' + '</ul>' + '</div>' + '<div id="kickass-weapons-menu" class="KICKASSELEMENT KICKASShidden" style="display:none">' + '<ul id="kickass-weapons-list" class="KICKASSELEMENT">' + '</ul>' + '</div>' + '<div id="kickass-pointstab-wrapper" class="KICKASSELEMENT">' + '<div id="kickass-points" class="KICKASSELEMENT">' + this.numPoints + '</div>' + '<div id="kickass-esctoquit" class="KICKASSELEMENT">Press esc to quit</div>' + this.getShareHTML() + '</div>' + '<ul id="kickass-pointstab-menu" class="KICKASSELEMENT" ' + (this.game.shouldShowMenu() ? '' : 'style="display:none"') + '>' + '<li class="KICKASSELEMENT"><a class="KICKASSELEMENT" id="kickass-link-highscores" href="#">Submit score</a></li>' + '<li class="KICKASSELEMENT"><a class="KICKASSELEMENT" id="kickass-link-menu" href="#">Menu</a></li>' + '<li class="last-li KICKASSELEMENT"><a class="KICKASSELEMENT" id="kickass-link-ships" href="#">Switch ship</a></li>' + '</ul>' + '</div>';
            ...
            var all = this.container.getElementsByTagName('img');
        ...
        getShareHTML: function() {
            ...
            //         return '<iframe class="KICKASSELEMENT kickass-like"></iframe>';
        ...
        navigateTo: function(page, dontShowMenu) {
            ...
            if (this.menu) {
                this.menu.socket.postMessage('navigate:!' + page);
            }
        },
        toggleMenu: function() {
            if (this.game.shouldShowMenu()) {
